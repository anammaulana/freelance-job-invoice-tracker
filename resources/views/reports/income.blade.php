<x-layouts.app title="Income Report">
    ...
    <div class="mb-6 grid gap-4 md:grid-cols-3">
        ...
        <div class="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
            <p class="text-sm text-zinc-600">Filtered expense total</p>
            <p class="mt-2 text-2xl font-semibold text-zinc-950">{{ number_format($totalExpenses, 2) }}</p>
        </div>
        <div class="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
            <p class="text-sm text-zinc-600">Net profit</p>
            <p class="mt-2 text-2xl font-semibold text-zinc-950">{{ number_format($netProfit, 2) }}</p>
        </div>
    </div>
    ...
    <section class="mt-8">
        <h2 class="text-lg font-semibold text-zinc-950">Expenses</h2>
        <div class="mt-3 overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm">
            @if ($expenses->isEmpty())
                <p class="p-5 text-sm text-zinc-600">Tidak ada expense pada rentang tanggal ini.</p>
            @else
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-zinc-200 text-sm">
                        <thead class="bg-zinc-100 text-left text-xs font-semibold uppercase tracking-wide text-zinc-600">
