<x-layouts.app title="New task">
    <div class="mb-6">
        ...
        <h1 class="mt-3 text-2xl font-semibold text-zinc-950">New task</h1>
        <p class="mt-1 text-sm text-zinc-600">{{ $project->name }}</p>
    </div>

    <form action="{{ route('projects.tasks.store', $project) }}" method="POST" class="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <div class="mt-6 flex justify-end gap-3">
            ...
            <button class="rounded-md bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-700">Save task</button>
        </div>
    </form>
</x-layouts.app>
